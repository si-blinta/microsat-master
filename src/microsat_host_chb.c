...
void check_and_restart(struct solver *S) {
    ...
    switch (S->config.rest_p) {
        case REST_DEFAULT:
            ...
            break;
        case REST_LUBY:
            if (S->config.last_conflicts >= luby(S->config.luby_base, S->config.luby_index))
            {
              printf("luby restart after %d conflicts\n",S->config.last_conflicts);
              restart(S); // Restart and update the averages
              S->config.luby_index++;
              S->config.last_conflicts = 0;
              if (reduce_flag)
                reduceDB(S, REDUCE_LIMIT); 
            }
            break;
        case REST_GEO:
            if (S->config.last_conflicts >= S->config.geo_max)
            {
            ...
            break;
        case REST_ARITH:
            if (S->config.last_conflicts >= S->config.arith_max)
            {
  ...
  {                              // Main solve loop
    ...
    { 
      ...
      if(S->config.br_p == BR_VSIDS){
        if(S->config.last_conflicts % S->config.decay_thresh_hold == 0)
          decay(S,0.99);
      }
      check_and_restart(S);
    }
    /*else if(conflict_flag == 0)
    {
      if(S->config.br_p == BR_CHB)
        S->config.multiplier = 0.9;
    }
    if(S->config.br_p == BR_CHB)
    {
      for(int i = 1; i < S->nVars+1 ; i++)
      {
        if(S->falses[i] == PROPAGATED || S->falses[-i] == PROPAGATED)
        {
          int reward = S->config.multiplier/(S->nConflicts - S->last_conflict[i] +1 );
          S->scores[i] = (1-S->config.alpha) * S->scores[i] + S->config.alpha*reward;
        if(S->falses[i] == PROPAGATED)
        {
          S->falses[i] = IMPLIED;
        }
        else if(S->falses[-i] == PROPAGATED)
        {
          S->falses[-i] = IMPLIED;
        }
        if(conflict_flag)
        {
            S->last_conflict[i] = S->nConflicts;
        }
        }
    ...
    if(S->config.br_p == BR_CHB)
    {
      if(S->config.alpha > 0.06 && conflict_flag == 1)
      {
        S->config.alpha *= 0.000006;    
      } 
      sort_variables(S);
    }*/
    ...
    {                                     // While there are watched clauses (watched by lit)
      ...
      { // If the clause is indeed unit
        ...
        { // If the other watched literal is falsified,
          ...
          /*if(S->config.br_p == BR_CHB)
            clause[0] = PROPAGATED;     // Mark it as propagated*/
        ...
        else
        {
          ...
          /*if(forced == 1)
            if(S->config.br_p == BR_CHB)
              lemma[0] = PROPAGATED;// Mark it as propagated ( asserted)*/
          break;
        }
      }
    }
...
void initCDCL(struct solver *S, int n, int m)
{
  ...
  S->config.alpha      = 0.4;    //Decreases by 0.000006 every conflicts => min = 0.06
  S->config.multiplier = 0.9;    // or 1.0;
  ...
  S->last_conflict = getMemory(S,n+1);
